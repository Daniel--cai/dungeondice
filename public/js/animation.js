...
class DiceRoll extends Animation{
  ...
  render(){
    ...
    shrink.end = function(){
      if (result[index][0] != CREST_SUMMON){
        player.updatePool(result[index][0],result[index][1])
      }
    }
...
class ButtonImageGrow extends Animation {
  constructor(index, name){
    ...
    this.duration = 2;
    //console.log('duration is', this.duration, 'index is ', index, shrink)
    this.sx = this.sy = 0;
    this.name = name
  }
  periodic(dt){
  ...
  render(){
    ...
    ctx.translate(x,y);
    ctx.drawImage(IMAGES[this.name+'Square'],0,0,this.sx, this.sy)
    if (this.text != null){
...
class DiceGrow extends Animation {
  constructor(index,crest, shrink = true){
    super()
    var size = this.size = 50;
  	var x = 100 + boardXPadding+size/2;
  	this.dx = this.dy = 100;
